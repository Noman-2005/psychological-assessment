                ...
                {Object.entries(personalityResult).map(([trait, score]) => (
                  <div key={trait} className="p-4 bg-slate-800/30 rounded-xl border border-slate-700/30 backdrop-blur-sm space-y-2">
                    ...
                    <p className="text-xs text-slate-400 font-light">
                      {(score as number) >= 4 ? (lang === 'bn' ? 'অত্যন্ত উচ্চ' : 'Very High') :
                       (score as number) >= 3.5 ? (lang === 'bn' ? 'উচ্চ' : 'High') :
                       (score as number) >= 2.5 ? (lang === 'bn' ? 'মাঝারি' : 'Moderate') :
                       (score as number) >= 1.5 ? (lang === 'bn' ? 'কম' : 'Low') :
