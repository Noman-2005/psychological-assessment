...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    if (data) {
      try {
        ...
        // Auto-fill report sections from assessment data
        setForm(prev => ({
          ...prev,
  ...
  const aiGenerate = async (sectionKey: keyof ReportData, label: string) => {
    ...
    try {
      // Use assessment data to generate relevant content
      const context = `
Client Number: ${form.clientNumber || "N/A"}
Reason for Referral: ${form.reasonForReferral || "N/A"}
Background Info: ${form.backgroundInfo || "N/A"}
Assessment Findings: ${assessmentData ? JSON.stringify(assessmentData.findings) : "N/A"}
      `.trim();

      // AI Responses based on assessment data
